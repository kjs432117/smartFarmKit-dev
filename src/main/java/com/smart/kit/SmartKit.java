...
//	(조명,급수정보,농약정보)
	...
	private String latestWaterTime = "N/A";
	private String latestPesTime = "N/A";
...
//	키트넘버, 작물, 필요시간, 작물유형 최조실행시 업데이트되도록 수정? 현재는 키트 종류별로 고정된 값을 가지고 있다고 가정
			...
			TimerTask m_task2 = new TimerTask() {
				...
				public void run() {
					...
					if (SmartKit.this.auto == 1) {
						eva.evaluateAll(SmartKit.this.temp, SmartKit.this.hum, SmartKit.this.light, SmartKit.this.water,
								SmartKit.this.pes);
						System.out.println("on auto growing");
					} else if (SmartKit.this.auto == 0) {
						currentTime = new Date();

						log.add(format.format(currentTime) + eva.evaluateAll(SmartKit.this.temp, SmartKit.this.hum,
								SmartKit.this.light, SmartKit.this.water, SmartKit.this.pes));
